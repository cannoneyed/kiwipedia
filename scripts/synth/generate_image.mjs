...
export async function generateStableDiffusionPrompt(title, caption) {
...
Please write a prompt for Stable Diffusion for an image captioned "${caption}", in the article for "${title}" in the english version of Wikipedia:`;
  const text = await system.getCompletion(promptText);
  return text;
}

export async function generateCaptionForSection(
  title,
  sectionTitle,
  sectionText,
) {
  const promptText = `The following is the "${sectionTitle} section for the article for "${title}" in the english version of Wikipedia:

${sectionText}

The following is a very short caption for the image in the "${sectionTitle} section of the article for "${title}" in the english version of Wikipedia: "`;
  let text = await system.getCompletion(promptText);
  if (text.endsWith('"')) {
...
export async function generateCaptionForPrompt(title, stableDiffusionPrompt) {
  const promptText = `Stable Diffusion is an AI art generation model similar to DALLE-2.
